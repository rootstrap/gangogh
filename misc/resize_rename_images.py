...
def resize_image(base_path, dest_path):
    ...
    # """
    TARGET_BASEWIDTH = 64
    image = imageio.imread(base_path)
    resize_image = Image.fromarray(image).resize(size=(TARGET_BASEWIDTH, TARGET_BASEWIDTH))
    # image = imageio.imresize(image,(TARGET_BASEWIDTH,TARGET_BASEWIDTH))
    imageio.imwrite(dest_path, resize_image)
